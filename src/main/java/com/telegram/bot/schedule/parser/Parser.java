...
public class Parser {
    ...
    public Parser(ScheduleRepository scheduleRepository) {
        this.scheduleRepository = scheduleRepository;
    }

    public String firstRovCol(Sheet sheet, int row, int col) {
    ...
    public void pars(int number) {
        try {
            ...
            int dateCol = 0;
            int timeCol = 1;
            Sheet sheet = wb.getSheetAt(NUMBER_LIST); // так как лист находится в 3 вкладке
                            ...
                            if (firstRovCol(sheet, row + 1, dateCol) != null) {
                                ...
                                if ((sheet.getRow(row).getCell(col).getCellType() != CellType.BLANK) || (sheet.getRow(row + 1).getCell(col).getCellType() != CellType.BLANK)) {
                                    ...
                                    Schedule us1 = Schedule.builder()
                                            ...
                                            .teacher(sheet.getRow(row).getCell(col).getStringCellValue())
                                            .subject(sheet.getRow(row + 1).getCell(col).getStringCellValue())
                                                    .build();
                                    Schedule save = scheduleRepository.save(us1);
